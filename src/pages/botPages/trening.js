    ...
    return (
        <ThemeProvider theme={darkTheme}>
            <Box sx = {{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <CssBaseline />
                <Typography sx = {{fontSize: 'large', padding: '30px 10px'}} variant="h4" gutterBottom>"{cat}"</Typography>
                        ...
                        {Object.keys(trening.categories[cat]).map((row, index) => (
                            ...
                            >
                                ...
                                <TableCell sx={{ padding: 0 }}>
                                    {!renameMode&&<IconButton sx={styleButton}
                                        onClick={()=>{setRenameMode(true); setRename(row)}}
                                    >
